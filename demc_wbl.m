%%  Function for DE-MC
function [para,Rhat,accrate,mix]= demc_wbl(Model_type,int_var,AMS,evl,bur,sts,cha,Covariate,int_cp_index) 
...
% Added code from demc_gp
if evalin('base', "exist('Weibull_Settings_Matrix', 'var')") && ~isempty(evalin('base', 'Weibull_Settings_Matrix'))
    % A settings matrix was found, so retrieve it.
    Weibull_Settings_Matrix = evalin('base', 'Weibull_Settings_Matrix');
    
    % Column 2 -> Scale
    scale_dist = Weibull_Settings_Matrix{1, 2};
    scale_p1   = Weibull_Settings_Matrix{2, 2};
    scale_p2   = Weibull_Settings_Matrix{3, 2};
    % Column 3 -> Shape
    shape_dist = Weibull_Settings_Matrix{1, 3};
    shape_p1   = Weibull_Settings_Matrix{2, 3};
    shape_p2   = Weibull_Settings_Matrix{3, 3};
else
...
% Preparing Initial Chains
[p_x_k,mix_k,x_k]=Prior_estimator(int_var(:,1:2),int_cp_index,AMS,Covariate,obs_int,shape_p2,cha,Model_type{1,1},shape_dist, shape_p1, shape_p2); %shape
[ p_x_sigma, mix_x_sig,x_sigma]=Prior_estimator(int_var(:,3:4),int_cp_index,AMS,Covariate,obs_int,scale_p2,cha,Model_type{2,1},scale_dist, scale_p1, scale_p2); %scale
% [ p_x_mu, mix_x_mu,x_mu]=Prior_estimator(int_var(:,5:6),int_cp_index,AMS,Covariate,obs_int,100,cha,Model_type{3,1});

non_xx= (log(wblpdf(obs_int,exp(x_k),exp(x_sigma))));
...
for i= 1:evl
    ...
    [p_z_k,z_k]=Prior_Simulation(int_var(:,1:2),int_cp_index,AMS,Covariate,obs_int,shape_p2,cha,z_z(:,1),Model_type{1,1},shape_dist, shape_p1, shape_p2);
    [p_z_sigma,z_sigma]=Prior_Simulation(int_var(:,3:4),int_cp_index,AMS,Covariate,obs_int,scale_p2,cha,z_z(:,2),Model_type{2,1},scale_dist, scale_p1, scale_p2);
    ...
    [p_z_k,z_k]=Prior_Simulation(int_var(:,1:2),int_cp_index,AMS,Covariate,obs_int,shape_p2,cha,z_z(:,1),Model_type{1,1},shape_dist, shape_p1, shape_p2);
    [p_z_sigma,z_sigma]=Prior_Simulation(int_var(:,3:4),int_cp_index,AMS,Covariate,obs_int,scale_p2,cha,z_z(:,2:end),Model_type{2,1},scale_dist, scale_p1, scale_p2);
    ...
    [p_z_k,z_k]=Prior_Simulation(int_var(:,1:2),int_cp_index,AMS,Covariate,obs_int,shape_p2,cha,z_z(:,1:m),Model_type{1,1},shape_dist, shape_p1, shape_p2);
    [p_z_sigma,z_sigma]=Prior_Simulation(int_var(:,3:4),int_cp_index,AMS,Covariate,obs_int,scale_p2,cha,z_z(:,m+1:end),Model_type{2,1},scale_dist, scale_p1, scale_p2);
   ...
   % [p_z_mu,z_mu]=Prior_Simulation(int_var(:,5:6),int_cp_index,AMS,Covariate,obs_int,100,cha,z_z(:,m+2:end),Model_type{3,1});
    ...
    non_zxx= (log(wblpdf(obs_int,exp(z_k),exp(z_sigma))));
