%%  Function for DE-MC
function [para,Rhat,accrate,mix]= demc_gp(Model_type,int_var,AMS,evl,bur,sts,cha,Covariate,int_cp_index) 
...
if evalin('base', "exist('GP_Settings_Matrix', 'var')") && ~isempty(evalin('base', 'GP_Settings_Matrix'))
    % A settings matrix was found, so retrieve it.
    GP_Settings_Matrix = evalin('base', 'GP_Settings_Matrix');
    
    % Column 2 -> Scale
    scale_dist = GP_Settings_Matrix{1, 2};
    scale_p1   = GP_Settings_Matrix{2, 2};
    scale_p2   = GP_Settings_Matrix{3, 2};
    % Column 3 -> Shape
    shape_dist = GP_Settings_Matrix{1, 3};
    shape_p1   = GP_Settings_Matrix{2, 3};
    shape_p2   = GP_Settings_Matrix{3, 3};
else
...
non_xx= (log(gppdf(obs_int,x_k,exp(x_sigma))));
...
for i= 1:evl
    ...
    [p_z_k,z_k]=Prior_Simulation(int_var(:,1:2),int_cp_index,AMS,Covariate,obs_int,0.3,cha,z_z(:,1),Model_type{1,1},shape_dist, shape_p1, shape_p2);
    [p_z_sigma,z_sigma]=Prior_Simulation(int_var(:,3:4),int_cp_index,AMS,Covariate,obs_int,100,cha,z_z(:,2:end),Model_type{2,1},scale_dist, scale_p1, scale_p2);
    ...
    % [p_z_k,z_k]=Prior_Simulation(int_var(:,1:2),int_cp_index,AMS,Covariate,obs_int,0.3,cha,z_z(:,1:m),Model_type{1,1});
    % [p_z_sigma,z_sigma]=Prior_Simulation(int_var(:,3:4),int_cp_index,AMS,Covariate,obs_int,100,cha,z_z(:,m+1:end),Model_type{2,1});
    ...
    non_zxx= (log(gppdf(obs_int,z_k,exp(z_sigma))));
