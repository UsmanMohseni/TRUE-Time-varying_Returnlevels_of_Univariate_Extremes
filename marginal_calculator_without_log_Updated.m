function [PDF,CDF,para,params]=marginal_calculator_without_log_Updated(DATA,para,Model_type,int_var,Covariate,best_fit)
[m,n]=size(Covariate);
    switch best_fit
        case 'gev'
        ...
        case 'gp'
            ...
            PDF=gppdf(DATA,exp(param(1,:))',(param(2,:))');
            CDF=gpcdf(DATA,exp(param(1,:))',(param(2,:))');
        ...
        case 'weibull'
            ...
            elseif(strcmp(Model_type(1,1),'Non-Stationary Trend') && strcmp(Model_type(2,1),'Non-Stationary Trend'))
             ...
             param(2,:)=median(scale_t);
              params{1}=shape;
            params{2}=scale_t;
              PDF=wblpdf(DATA,exp(param(1,:))',exp(param(2,:))');
              CDF=wblcdf(DATA,exp(param(1,:))',exp(param(2,:))');
        ...
        case 'normal'
            ...
            params{2}=scale_t;
              ...
              PDF=normpdf(DATA,(param(1,:))',exp(param(2,:))');
              CDF=normcdf(DATA,(param(1,:))',exp(param(2,:))');
        ...
        case 'ev'
            ...
            params{2}=scale_t;
              ...
              PDF=evpdf(DATA,(param(1,:))',exp(param(2,:))');
              CDF=evcdf(DATA,(param(1,:))',exp(param(2,:))');
          ...
          case 'logistic'
            ...
            % --------------------------------------------------
            mu = param(1,:)';
            s  = exp(param(2,:)');   % scale > 0
        
            PDF = exp(-(DATA-mu)./s) ./ ( s .* (1 + exp(-(DATA-mu)./s)).^2 );
            CDF = 1 ./ (1 + exp(-(DATA-mu)./s));
