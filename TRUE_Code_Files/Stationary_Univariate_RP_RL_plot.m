function Stationary_Univariate_RP_RL_plot(params,best_fit,Time,Label,no)
    ...
    switch best_fit
        ...
        case 'normal'
            ...
            for i = 2:1:100
                RL1(i - 1, :) = (norminv(1 - 1 / i, mu(1, 1), mu(2, 1)));
                RL2(i - 1, :) = (norminv(1 - 1 / i, CI(1, 1), CI(2, 1)));
                RL3(i - 1, :) = (norminv(1 - 1 / i, CI(1, 2), CI(2, 2)));
            end
            for i = 2:1:100
                RL4(i - 1, :) = (norminv(1 - 1 / i, CI2(1, 1), CI2(2, 1)));
                RL5(i - 1, :) = (norminv(1 - 1 / i, CI2(1, 2), CI2(2, 2)));
            end
        ...
        case 'ev'
            ...
            for i = 2:1:100
                RL1(i - 1, :) = (evinv(1 - 1 / i, mu(1, 1), mu(2, 1)));
                RL2(i - 1, :) = (evinv(1 - 1 / i, CI(1, 1), CI(2, 1)));
                RL3(i - 1, :) = (evinv(1 - 1 / i, CI(1, 2), CI(2, 2)));
            end
            for i = 2:1:100
                RL4(i - 1, :) = (evinv(1 - 1 / i, CI2(1, 1), CI2(2, 1)));
                RL5(i - 1, :) = (evinv(1 - 1 / i, CI2(1, 2), CI2(2, 2)));
            end
        ...
        case 'weibull'
            ...
            for i = 2:1:100
                RL1(i - 1, :) = (wblinv(1 - 1 / i, mu(1, 1), mu(2, 1)));
                RL2(i - 1, :) = (wblinv(1 - 1 / i, CI(1, 1), CI(2, 1)));
                RL3(i - 1, :) = (wblinv(1 - 1 / i, CI(1, 2), CI(2, 2)));
            end
            for i = 2:1:100
                RL4(i - 1, :) = (wblinv(1 - 1 / i, CI2(1, 1), CI2(2, 1)));
                RL5(i - 1, :) = (wblinv(1 - 1 / i, CI2(1, 2), CI2(2, 2)));
            end
        ...
        case 'logistic'
            ...
            mu_samp    = mu_samp(:, X);
            log_s_samp = log_s_samp(:, X);
        
            % Final parameter samples
            para = [mu_samp exp(log_s_samp)];   % [mu , s]
            ...
            % --------------------------------------------------
            % Return levels (Logistic inverse CDF)
            % --------------------------------------------------
            for i = 2:100
                p = 1 - 1/i;
                RL1(i-1,:) = mu(1,1) + mu(2,1)*log(p/(1-p));
                RL2(i-1,:) = CI(1,1) + CI(2,1)*log(p/(1-p));
                RL3(i-1,:) = CI(1,2) + CI(2,2)*log(p/(1-p));
            end
        
            for i = 2:100
                p = 1 - 1/i;
                RL4(i-1,:) = CI2(1,1) + CI2(2,1)*log(p/(1-p));
                RL5(i-1,:) = CI2(1,2) + CI2(2,2)*log(p/(1-p));
            end
    ...
    title('Stationary: Return Period vs Return Level', 'FontSize', 14, 'FontWeight', 'bold', 'FontName', 'Times New Roman')
    ...
    grid on
set(gcf, 'Color', 'w', 'Units', 'centimeters', 'Position', [0.29, 4.25, 17.75,7.6]);
