function F = fitmethis2_Updated(data,selected_distributions,varargin)
...
% Distributions
Cdist = selected_distributions;
%{'gamma'; 'gev';'gp'; 'lognormal';'weibull';'logistic';'ev';'normal'};
mustbepos = 11;
        ...
        for j = 1:numel(Cdist)
            ...
            % Any other case do the fit ...
                ...
                switch Cdist{j}
                    ...
                    case 'weibull'
                        params=wblfit(data);
                        cdf=wblcdf(data,params(1),params(2));
                    [~,F(j).kspval, F(j).ksstat,~] = kstest(data,"CDF",[data cdf]);
                    case 'normal'
                        params=normfit(data);
                        cdf=normcdf(data,params(1),params(2));
                    [~,F(j).kspval, F(j).ksstat,~] = kstest(data,"CDF",[data cdf]);
                    case 'ev'
                        params=evfit(data);
                        cdf=evcdf(data,params(1),params(2));
                    [~,F(j).kspval, F(j).ksstat,~] = kstest(data,"CDF",[data cdf]);
                     case 'logistic'
                    pd = fitdist(data,'Logistic');      % fit logistic distribution
                    cdf = cdf(pd, data);                % logistic CDF
                    [~, F(j).kspval, F(j).ksstat, ~] = kstest(data, "CDF", [data cdf]);
