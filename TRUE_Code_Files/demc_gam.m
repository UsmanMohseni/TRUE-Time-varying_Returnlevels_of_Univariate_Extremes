%%  Function for DE-MC
function [para,Rhat,accrate,mix]= demc_gam(Model_type,int_var,AMS,evl,bur,sts,cha,Covariate,int_cp_index) 
...
if evalin('base', "exist('Gamma_Settings_Matrix', 'var')") && ~isempty(evalin('base', 'Gamma_Settings_Matrix'))
    % A settings matrix was found, so retrieve it.
    Gamma_Settings_Matrix = evalin('base', 'Gamma_Settings_Matrix');
    
    % Column 2 -> Scale
    scale_dist = Gamma_Settings_Matrix{1, 2};
    scale_p1   = Gamma_Settings_Matrix{2, 2};
    scale_p2   = Gamma_Settings_Matrix{3, 2};
    % Column 3 -> Shape
    shape_dist = Gamma_Settings_Matrix{1, 3};
    shape_p1   = Gamma_Settings_Matrix{2, 3};
    shape_p2   = Gamma_Settings_Matrix{3, 3};
else
...
non_xx= (log(gampdf(obs_int,exp(x_k),exp(x_sigma))));
...
for i= 1:evl
   ...
   % [p_z_mu,z_mu]=Prior_Simulation(int_var(:,5:6),int_cp_index,AMS,Covariate,obs_int,100,cha,z_z(:,m+2:end),Model_type{3,1});
    ...
    non_zxx= (log(gampdf(obs_int,exp(z_k),exp(z_sigma))));
