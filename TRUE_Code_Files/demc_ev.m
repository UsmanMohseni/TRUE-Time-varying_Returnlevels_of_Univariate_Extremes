%%  Function for DE-MC
function [para,Rhat,accrate,mix]= demc_ev(Model_type,int_var,AMS,evl,bur,sts,cha,Covariate,int_cp_index) 
...
if evalin('base', "exist('Gumbel_Settings_Matrix', 'var')") && ~isempty(evalin('base', 'Gumbel_Settings_Matrix'))
    % A settings matrix was found, so retrieve it.
    Gumbel_Settings_Matrix = evalin('base', 'Gumbel_Settings_Matrix');
    
    % Column 1 -> Location
    loc_dist = Gumbel_Settings_Matrix{1, 1};
    loc_p1   = Gumbel_Settings_Matrix{2, 1};
    loc_p2   = Gumbel_Settings_Matrix{3, 1};
    % Column 2 -> Scale
    scale_dist = Gumbel_Settings_Matrix{1, 2};
    scale_p1   = Gumbel_Settings_Matrix{2, 2};
    scale_p2   = Gumbel_Settings_Matrix{3, 2};
else
...
non_xx= (log(evpdf(obs_int,x_k,exp(x_sigma))));
...
for i= 1:evl
    ...
    non_zxx= (log(evpdf(obs_int,z_k,exp(z_sigma))));
