%%  Function for DE-MC
function [para,Rhat,accrate,mix]= demc_normal(Model_type,int_var,AMS,evl,bur,sts,cha,Covariate,int_cp_index) 
...
if evalin('base', "exist('Normal_Settings_Matrix', 'var')") && ~isempty(evalin('base', 'Normal_Settings_Matrix'))
    % A settings matrix was found, so retrieve it.
    Normal_Settings_Matrix = evalin('base', 'Normal_Settings_Matrix');
    
    % Column 1 -> Location
    loc_dist = Normal_Settings_Matrix{1, 1};
    loc_p1   = Normal_Settings_Matrix{2, 1};
    loc_p2   = Normal_Settings_Matrix{3, 1};
    % Column 2 -> Scale
    scale_dist = Normal_Settings_Matrix{1, 2};
    scale_p1   = Normal_Settings_Matrix{2, 2};
    scale_p2   = Normal_Settings_Matrix{3, 2};
else
...
non_xx= (log(normpdf(obs_int,x_k,exp(x_sigma))));
...
for i= 1:evl
    ...
    non_zxx= (log(normpdf(obs_int,z_k,exp(z_sigma))));
