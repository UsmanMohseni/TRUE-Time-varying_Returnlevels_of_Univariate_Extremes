function [DIC1,DIC2,PARA_FINAL]= model_evaluator2_Updated(numVariables,allCombinations,Covariate1,AMS,best_fit)
warning('off', 'all');
com=1;
PARA_FINAL(1:25005,1:15,1:63)=nan;


[~,Model_type1,~]=initial_var_Updated(AMS,Covariate1,best_fit,20);
...
if(any(strcmp(Model_type1(:,1),'Non-Stationary Trend')))
    ...
    [int_var,Model_type,int_MW_par]=initial_var_Updated(AMS,Covariate,best_fit,20);
    int_var=Variable_parameters1_Updated(int_var,Model_type,Change_point,AMS,int_MW_par,Covariate,best_fit);
    ...
    bur=10000;
    sts=1;
    [para,Rhat,~,~]= demc_all_Updated(Model_type,int_var,AMS,evl,bur,sts,cha,Covariate,NaN,best_fit);
    x=1;
     ...
     switch best_fit 
         ...
         case 'normal'
              % Taking exp of SD parameters;
                ...
                likelihood(1:size(AMS,1),1)=NaN;
                for s=1:1:size(AMS,1)
                    likelihood(s,1)=normpdf(AMS(sts,1),param(1,s),exp(param(2,s)));
                end
         ...
         case 'ev'
              % Taking exp of SD parameters;
                ...
                likelihood(1:size(AMS,1),1)=NaN;
                for s=1:1:size(AMS,1)
                    likelihood(s,1)=evpdf(AMS(sts,1),param(1,s),exp(param(2,s)));
                end
         ...
         case 'weibull'
              % Taking exp of a and b parameters;
                ...
                likelihood(1:size(AMS,1),1)=NaN;
                for s=1:1:size(AMS,1)
                    likelihood(s,1)=wblpdf(AMS(sts,1),exp(param(1,s)),exp(param(2,s)));
                end
        ...
        case 'logistic'
            ...
            para1 = para;   % retained as in original
            PARA_FINAL(1:25005,1:size(para,2),com) = para;
        
            
            % Construct time-varying parameters
            
            if strcmp(Model_type(1,1),'Stationary') && strcmp(Model_type(2,1),'Stationary')
                mu_t = repmat(para(:,1),1,m);
                log_s_t = repmat(para(:,2),1,m);
        
            elseif strcmp(Model_type(1,1),'Stationary') && strcmp(Model_type(2,1),'Non-Stationary Trend')
                mu_t = repmat(para(:,1),1,m);
                log_s_t = para(:,2:end-1)*Covariate' + para(:,end);
        
            elseif strcmp(Model_type(1,1),'Non-Stationary Trend') && strcmp(Model_type(2,1),'Stationary')
                mu_t = para(:,1:end-2)*Covariate' + para(:,end-1);
                log_s_t = repmat(para(:,end),1,m);
        
            elseif strcmp(Model_type(1,1),'Non-Stationary Trend') && strcmp(Model_type(2,1),'Non-Stationary Trend')
                mu_t = para(:,1:n)*Covariate' + para(:,n+1);
                log_s_t = para(:,n+2:end-1)*Covariate' + para(:,end);
            end
        
            
            % Posterior median parameters
            
            param(1,:) = median(mu_t);        % location
            param(2,:) = median(log_s_t);     % log-scale
        
           
            % Logistic likelihood
            
            likelihood = NaN(size(AMS,1),1);
        
            for s = 1:size(AMS,1)
                mu = param(1,s);
                sc = exp(param(2,s));   % scale > 0
                likelihood(s,1) = logistic_logpdf(AMS(sts,1), mu, sc);
            end
             ...
             [DIC3]=DIC_Updated(para1(bur+1:end,:),Covariate,AMS,Model_type,best_fit);
             DIC1(com,1)=DIC3;
...
[int_var,Model_type,int_MW_par]=initial_var_Updated(AMS,Covariate1,best_fit,20);
int_var=Variable_parameters1_Updated(int_var,Model_type,NaN,AMS,int_MW_par,Covariate1,best_fit);
...
bur=10000;
sts=1;
[para,Rhat,~,~]= demc_all_Updated(Model_type,int_var,AMS,evl,bur,sts,cha,Covariate1,NaN,best_fit);
x=1;
 while max(Rhat)>1.1||any(isnan(Rhat))%||accrate_PF<0.3
     x=x+1;
     if(x<=5)
...
DIC1=DIC_Updated(para1(bur+1:end,:),Covariate1,AMS,Model_type,best_fit);
DIC2=sprintf('NaN');
