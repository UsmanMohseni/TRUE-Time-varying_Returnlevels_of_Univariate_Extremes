...
% Usman Mohseni: mohseni_ua@ce.iitr.ac.in  or 
% Dr. R Vinnarasi: vinnarasi@ce.iitr.ac.in 
...
% finalsave = "E:\TRUE\All_Results_1.mat"; % workspace save location
% 
% Total_simulations = 2; % Total number of simulations (modifiable by the user as required)


%% Load AMS data and covariates
% 
list_Covariates = dir(fullfile(File_path_Covariates, '*.csv'));
...
% fit_PF=fitmethis2_Updated(PF);
%  fit_PF = fit_PF(~strcmpi({fit_PF.name}, 'gamma')); % remove gamma
% %  fit_PF = fit_PF(~strcmpi({fit_PF.name}, 'weibull')); % remove gamma
% %  fit_PF = fit_PF(~strcmpi({fit_PF.name}, 'gev')); % remove gamma
% 
...
%     [DIC_PF1(:,simulation),~,PARA_FINAL_PF] = model_evaluator2_Updated(numVariables, allCombinations, Covariate1, PF, best_fit_PF);
...
switch best_fit_PF 
    ...
    otherwise
        warning('Unknown distribution: %s', best_fit_pf);
end
...
[int1_var,Model_type1,int1_MW_par,Z_PF] = initial_var_Updated(PF, Covariate(:,Best_Covariate_PF), best_fit_PF, 20);
int1_var = Variable_parameters1_Updated(int1_var, Model_type1, Change_point, PF, int1_MW_par, Covariate(:,Best_Covariate_PF), best_fit_PF);

para_PF = PARA_FINAL_PF1(:,:,Best_Model_PF);
count_PF = numel(int1_var) - sum(isnan(int1_var),'all');
para_PF = para_PF(:,1:count_PF);

[PDF_PF, CDF_PF, para_PF, params_PF] = marginal_calculator_Updated(PF, para_PF, Model_type1, int1_var, Covariate(:,Best_Covariate_PF), best_fit_PF);

%% Stationary modelling

runWindow();

cha = Run_Settings_Matrix{1,1}; %chains
evl = Run_Settings_Matrix{2,1}; %iterations
bur = Run_Settings_Matrix{3,1}; %burns


should_plot = Run_Settings_Matrix{4, 1}; % Is true if user checked "Yes" for plots
should_save = Run_Settings_Matrix{5, 1}; % Is true if user checked "Yes" for save


% cha = 5; evl = 15000; bur = 10000; 
sts = 1;
[sta_int1_var,sta_Model_type1,sta_int1_MW_par,~] = initial_var_Updated(PF, Covariate(:,1), best_fit_PF, 20);
sta_int1_var(2,:) = NaN;
sta_Model_type1(:,1) = {'Stationary'};
sta_int1_var = Variable_parameters1_Updated(sta_int1_var, sta_Model_type1, Change_point, PF, sta_int1_MW_par, Covariate(:,Best_Covariate_PF), best_fit_PF);

[sta_para_PF,sta_Rhat_PF,~,~] = demc_all_Updated(sta_Model_type1, sta_int1_var, PF, evl, bur, sts, cha, Covariate(:,1), NaN, best_fit_PF);
while max(sta_Rhat_PF) > 1.1 || any(isnan(sta_Rhat_PF))
    [sta_para_PF,sta_Rhat_PF,~,~] = demc_all_Updated(sta_Model_type1, sta_int1_var, PF, evl, bur, sts, cha, Covariate(:,1), NaN, best_fit_PF);
end

[sta_PDF_PF, sta_CDF_PF,~,sta_params_PF] = marginal_calculator_without_log_Updated(PF, sta_para_PF, sta_Model_type1, sta_int1_var, Covariate(:,1), best_fit_PF);
...
if should_save
    ...
    disp('Skipping file save as per user selection.');
end
toc
%% Plots 
...
if should_plot
   ...
   %% Figure 4: Effective Return Level Plot 

figure(4);

gca=figure(4);
set(gcf, 'Units', 'centimeters', 'Position', [9, 0.92, 19.5, 17]);

tiledlayout(1,2,"TileSpacing","tight","Padding","none")


% Discharge (blue)
[sta_PF_RL, sta_PF_RL1, sta_PF_RL2, sta_PF_RL3, sta_PF_RL4, sta_PF_RL5] = Return_level_plot_3(sta_params_PF, PF, Years, best_fit_PF, " Discharge (m^3/sec)", 0, 4, 0, 0, 0);
[PF_RL, PF_RL1, PF_RL2, PF_RL3, PF_RL4, PF_RL5] = Return_level_plot_3(params_PF, PF, Years, best_fit_PF, "Discharge (m^3/sec)", 1, 3, 0, 0, 1);



p_discharge = plot(nan, nan, '-', 'Color', [0.2, 0.5, 0.9], 'LineWidth', 1.5);  % Blue
...
for i = 1:6
    p_RP(i) = plot(nan, nan, '-.', 'Color', RP_COLORS(i,:), 'LineWidth', 1.4);
end

lgd = legend([p_RP, p_discharge], ...
             {'2-Year RP', '5-Year RP', '10-Year RP', ...
              '25-Year RP', '50-Year RP', '100-Year RP', ...
              'Discharge', 'River Discharge', 'Sea Level'}, ...
...
lgd.Layout.Tile = 'south';   % center legend below all tiles

 fullPath = fullfile(fileLocation, sprintf('%s_%d', 'Time_Varying_RL',4)); 
%     Save in PNG format (high resolution)
    print(fullPath, '-dpng', '-r600');
%     Save in TIFF format
    print(fullPath, '-dtiff', '-r600'); 
%     Save in FIG format (MATLAB figure)
    savefig(fullPath); 
%     Save in EMF format (for compatibility with MS Office)
    print(fullPath, '-dmeta'); 

    %% Figure 5 & 6: Return Period Vs Return Level Plots 

    Stationary_Univariate_RP_RL_plot(sta_params_PF,best_fit_PF,Time,'Return Level (Discharge m^3/s)',5)
    
    fullPath = fullfile(fileLocation, sprintf('%s_%d', 'S_RP_RL_plot_univariate',5)); 
        % Save in PNG format (high resolution)
        print(fullPath, '-dpng', '-r600');
        % Save in TIFF format
        print(fullPath, '-dtiff', '-r600'); 
    ...
    Non_Stationary_Univariate_RP_RL_plot(params_PF,best_fit_PF,Time,'Return Level (Discharge m^3/s)',6)
    
    fullPath = fullfile(fileLocation, sprintf('%s_%d', 'NS_RP_RL_plot_univariate',6)); 
        % Save in PNG format (high resolution)
        print(fullPath, '-dpng', '-r600');
        % Save in TIFF format
        print(fullPath, '-dtiff', '-r600'); 
    ...
    disp('Skipping plot generation as per user selection.');
end